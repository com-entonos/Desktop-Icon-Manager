...
class DockHelper {
    ...
    func setDockTile(_ dockTile: NSDockTile?) {
        ...
        if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("bundleID=\(self.BundleID, privacy: .public)< this bundleID=\(Bundle(for: DockHelper.self).bundleIdentifier ?? "nothing", privacy: .public)<- \(dockTile == nil ? "no dockTile":dockTile!.className, privacy: .public)< isMain? \(self.isMain, privacy: .public)") }
    }
    
    func setMenu(_ arrangements : [String]? = nil) -> NSMenu? {
        if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("isMain? \(self.isMain, privacy: .public), bundlID=\(self.BundleID, privacy: .public)< this bundleID=\(Bundle(for: DockHelper.self).bundleIdentifier ?? "nothing", privacy: .public)< \(self.isMain, privacy: .public)<") }
        ...
        if arrangements != nil { allArrangements = arrangements! } else {
            ...
            if let a = CFPreferencesCopyAppValue("orderedArrangements" as CFString, BundleID as CFString) as? [String] {
                allArrangements = a
                if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("orderedArrangements=\(a, privacy: .public)<") }
            } else {
                if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("no orderedArrangements in \(self.BundleID, privacy: .public)") }
                return nil
            }
        }
        ...
        if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("return menu: \(menu.numberOfItems, privacy: .public)") }
        return menu
    }
    @objc func selectDMI(_ sender: NSMenuItem) {
        ...
        if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("item called \(sender.title, privacy: .public) \(sender.tag, privacy: .public) \(name, privacy: .public)") }
        if NSWorkspace.shared.runningApplications.first(where: { app in app.bundleIdentifier == BundleID}) != nil {
            ...
            if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("send notification \(sender.title, privacy: .public) ->\(cmd0, privacy: .public), obj=\(sender.tag < 0 ? "nil" : name, privacy: .public)<-") }
            ...
            if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("send zsh command ->\(cmd0, privacy: .public)<-") }
            try? saveShell(cmd0)
        }
    }
