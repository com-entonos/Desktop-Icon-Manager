...
class DIMDockTilePlugin: NSObject, NSDockTilePlugIn {
   ...
   //     if let list = notice.object as? [String] { UserDefaults.standard.set(list, forKey: "orderedArrangements")} })
    @objc func saveUserDefaults(_ notice: Notification) {
        if #available(macOS 11.0, *) { Logger(subsystem: SUBSYSTEM, category: "info").log("DIMDockTilePlugin distributedNot newArrangement recieved=\(notice.name.rawValue ,privacy: .public)<>\(notice.userInfo?["orderedArrangements"] as? [String] ?? [""],privacy: .public)<") }
        if let list = notice.userInfo?["orderedArrangements"] as? [String] { UserDefaults.standard.set(list, forKey: "orderedArrangements") }
    }
    override init() {
        super.init()
        if #available(macOS 11.0, *) { Logger(subsystem: SUBSYSTEM, category: "info").log("DIMDockTilePlugin.init Bundle=\(Bundle(for: DIMDockTilePlugin.self).bundleIdentifier ?? "no bundle",privacy: .public)<>\(Bundle.main.bundleIdentifier ?? "no main bundle",privacy: .public)< \(NSWorkspace.shared.runningApplications.map {$0.bundleIdentifier},privacy: .public)") }
    ...
    func setDockTile(_ dockTile: NSDockTile?) {
        if #available(macOS 11.0, *) { Logger(subsystem: SUBSYSTEM, category: "info").log("DIMDockTilePlugin.setDockTile \(dockTile?.className ?? "", privacy: .public)") }
        if let _ = dockTile {
            NSWorkspace.shared.notificationCenter.addObserver(forName: NSNotification.Name("newArrangement"), object: nil, queue: .main, using: { notice in
                if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("DIMDockTilePlugin.WSnotification newArrangement recieved:\(notice.object as? [String] ?? ["nil"], privacy: .public)<")}
                if let list = notice.object as? [String] { UserDefaults.standard.set(list, forKey: "orderedArrangements")} })
            NotificationCenter.default.addObserver(forName: NSNotification.Name("newArrangement"), object: nil, queue: .main, using: { notice in
                if #available(macOS 11.0, *) { Logger(subsystem: "DIMDTP", category: "info").log("DIMDockTilePlugin.notification newArrangement recieved:\(notice.object as? [String] ?? ["nil"], privacy: .public)<")}
                if let list = notice.object as? [String] { UserDefaults.standard.set(list, forKey: "orderedArrangements")} })
        } else {
            ...
            if #available(macOS 11.0, *) { Logger(subsystem: SUBSYSTEM, category: "info").log("DIMDockTilePlugin.notification stopped")}
        }
        dockHelper.setDockTile(dockTile)
    }
    func dockMenu() -> NSMenu? {
        if #available(macOS 11.0, *) { Logger(subsystem: SUBSYSTEM, category: "info").log("DIMDockTilePlugin.dockMenu") }
 ...
 /*   init(_ bundle : String = "com.parker9.DIM-4") {
    ...
    func setMenu(_ arrangements : [String]? = nil) -> NSMenu? {
        ...
        if arrangements != nil { allArrangements = arrangements! } else {
            ...
            if let a = UserDefaults.standard.array(forKey: "orderedArrangements") as? [String] {
                allArrangements = a
                if #available(macOS 11.0, *) { Logger(subsystem: subsystem, category: "info").log("  from UserDefaults: \(self.allArrangements.count,privacy: .public) orderedArrangements=\(self.allArrangements, privacy: .public)<>\(self.BundleID, privacy: .public)<") }
            } else if let a = CFPreferencesCopyAppValue("orderedArrangements" as CFString, BundleID as CFString) as? [String] {
                //if let a = CFPreferencesCopyValue("orderedArrangements" as CFString, BundleID as CFString, kCFPreferencesCurrentUser,kCFPreferencesCurrentHost) as? [String] {
                    allArrangements = a
                    if #available(macOS 11.0, *) { Logger(subsystem: subsystem, category: "info").log("  from CFPrefs: \(self.allArrangements.count,privacy: .public) orderedArrangements=\(self.allArrangements, privacy: .public)<>\(self.BundleID, privacy: .public)<") }
            } else {
                if #available(macOS 11.0, *) { Logger(subsystem: subsystem, category: "info").log("  no orderedArrangements in \(self.BundleID, privacy: .public)") }
